...
export default function Home() {
  ...
  useEffect(() => {
    async function loadMovies() {
      setLoading(true);
      try {
        // This is the secret! We ask Vercel's backend directly instead of TMDB
        const res = await fetch('/api/tmdb', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ language, platform, genre })
        });
        const liveMovies = await res.json();
        setMovies(liveMovies);
      } catch (e) {
        console.error("Failed to route through backend", e);
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="container" style={{ paddingTop: '2rem', paddingBottom: '4rem' }}>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '4rem 0', color: 'var(--text-secondary)' }}>
          <h3 className="title">Bypassing Network Blocks... 🍿</h3>
        </div>
      ) : (
        <>
          ...
          <section>
            ...
            <p className="subtitle" style={{ marginBottom: '2rem' }}>Showing live results matching your filters.</p>
...
export default function Home() {
  ...
  // This powerful hook listens to your dropdowns and fetches live from TMDB!
  useEffect(() => {
    async function loadMovies() {
      setLoading(true);
      try {
  ...
  return (
    <div className="container" style={{ paddingTop: '2rem', paddingBottom: '4rem' }}>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '4rem 0', color: 'var(--text-secondary)' }}>
          <h3 className="title">Scanning TMDB Databases... 🍿</h3>
        </div>
      ) : (
        <>
          <section style={{ marginBottom: '3rem' }}>
          ...
          <section>
            <h2 className="title" style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>Full OTT Release List</h2>
            <p className="subtitle" style={{ marginBottom: '2rem' }}>Showing {movies.length} live results from your filters.</p>
